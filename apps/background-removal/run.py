...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Background Removal / Matting Comparator")
    ...
    cfg["methods"] = list(resolved["methods"])

    output = args.output.resolve()
    record_run_contract(
        app_name="background-removal", app_dir=ROOT, output_dir=output,
        structural_config=args.structural_config.resolve(), controls_definition=controls_path,
        resolved=resolved,
    )
    runtime_cfg = write_runtime_config(cfg)
    try:
        cmd = [sys.executable, str(ROOT / "compare_backgrounds.py"), str(args.input), str(args.output), "--config", str(runtime_cfg)]
